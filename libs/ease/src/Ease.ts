...
export class Ease {
    ...
    public from(value: number, callback?: (n: number) => void) {
        const step: EaseStep = EaseStep.Get();
        step.init(EaseStepType.FROM, value, 0);
        this._addStep(step);
        return this;
    }
    public to(value: number, timems: number = 1000, easeFunction: (t: number) => number = EaseFunction.Linear, callback?: (n: number) => void) {
        const step: EaseStep = EaseStep.Get();
    ...
    public get value(): number { return this._currentValue; }
    //#endregion
