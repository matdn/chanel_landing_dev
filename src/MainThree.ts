...
export class MainThree {
    ...
    public static SetDomElementContainer(element: HTMLElement | null): void {
        ...
        if (this._DomElementContainer) {
            ...
            if (this._IsStart) InteractiveManager.Start(this._DomElementContainer);
            this._Resize();
    ...
    private static _CreateScene() {
        ...
        if (this._CameraController && this._CameraController.camera) {
           ...
           // console.log("hello");
            ...
            this._RenderPass = new RenderPass(this._Scene, this._CameraController.camera);
            this._Composer.addPass(this._RenderPass);
        }
        const bloomPass = new UnrealBloomPass(new Vector2(window.innerWidth, window.innerHeight), 2.2, 0.8, 0.005);
        const v = new Vector3(0.6,0.4,0.2);
        bloomPass.bloomTintColors = [v, v, v, v, v];
    ...
    public static Start() {
        ...
        if (this._DomElementContainer) InteractiveManager.Start(this._DomElementContainer);
    ...
    private static _Resize = (): void => {        
        if (this._Renderer && this._CameraController && this._DomElementContainer) {
            ThreeUtils.ResizeToDom(this._DomElementContainer, this._Renderer, this._CameraController.camera);
        }
        if (this._DomElementContainer) {
    ...
    public static get DomElementContainer(): HTMLElement | null { return this._DomElementContainer; }
    public static get DomRect(): DOMRect { return this._DOMRect; }
    public static get CameraController(): CameraControllerBase | null { return this._CameraController; }
    //#endregion
